...
max_n_data = 100 # range for observational data size
...
if (sd_exp == 1 and sd_obs == 1) or (sd_exp == 10 and sd_obs == 10):
    ...
    if sd_exp == 1 and sd_obs == 1:
        plt.text(n_data_vals[10], y_thresh+0.27/100, 'Log scale', fontsize=12, color='grey', va='center', ha='left')
        ...
        plt.text(-10, 1, r'$\times 10^{-2}$', fontsize=10, ha='left', va='bottom')
        plt.yticks(yticks, ylabels, fontsize=12)
    elif sd_exp == 10 and sd_obs == 10:
        plt.text(n_data_vals[10], y_thresh+0.3, 'Log scale', fontsize=12, color='grey', va='center', ha='left')
        plt.text(n_data_vals[10], y_thresh-0.03, 'Linear scale', fontsize=12, color='grey', va='center', ha='left')
        yticks = [1, 2, 3, 4, 5, 20, 50, 80]
        yticks = np.linspace(np.min(ours_cv_mean), y_thresh, 5).tolist() + [2.5, 5, 10, 20, 40, 80]
        plt.yticks(yticks, fontsize=12)
    
    plt.xticks([1, 21, 41, 61, 81, 101], fontsize=12)
    plt.legend(fontsize=12)
    plt.tight_layout()
